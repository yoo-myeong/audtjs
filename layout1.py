...
display = Entry(window, width= 15, justify ="right", show = '*') # justify = "right" 숫자가 오른쪽으로부터
...
def doorlock():
    ...
    for i in mylist:
        ...
        button_dat = Button(window, text='*', height=2, width=10, command=lambda: cal('*'))
        button_equal = Button(window, text='#', height=2, width=10, command=lambda: cal('#'))
        ...
        Button(window, text='뒤로가기', width=20, command=lambda:cal2('화면을 터치해주세요')).place(x=650, y=100)

        label = Entry(window, width=24, justify='right')
        label.place(x=640, y=50)
...
def cal2(key):
    if key=="화면을 터치해주세요":
        ...
        for i in mylist:
            ...
            call=PhotoImage(file="md_b.gif")
            w_lb= Button(window,height=100,width=100,image=call)
            w_lb.place(x=600,y=100)
...
button1 = Button(window, text='화면을 터치해주세요', command=lambda: cal2('화면을 터치해주세요')).place(x=330,y=220)
